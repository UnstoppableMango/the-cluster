...
import { ServerClass } from '@unstoppablemango/thecluster-crds/metal/v1alpha2';
import { Node, Versions } from './config';
import * as YAML from 'yaml';
import { TalosControlPlane } from '@unstoppablemango/thecluster-crds/controlplane/v1alpha3';
import { MetalMachineTemplate } from '@unstoppablemango/thecluster-crds/infrastructure/v1alpha3';
import { output } from '@pulumi/pulumi';
import { TalosConfigTemplate } from '@unstoppablemango/thecluster-crds/bootstrap/v1alpha2';

const amdZfsEnv = 'amd-zfs'; // TODO: Stack ref

const apolloServerClass = new ServerClass('apollo', {
...
const zeusServerClass = new ServerClass('zeus', {
  metadata: { name: 'zeus' },
  spec: {
    environmentRef: {
      name: amdZfsEnv,
...
const thing = new TalosConfigTemplate('thing', {
  metadata: { name: '' },
  spec: {
    template: {
      spec: {
...
const metalMachine = new MetalMachineTemplate('zeus', {
  metadata: { name: 'zeus' },
  spec: {
    template: {
      spec: {
        serverRef: {
          name: output(zeusServerClass.metadata).apply(x => x?.name ?? ''),
        },
      },
    },
  },
...
const controlPlane = new TalosControlPlane('pinkdiamond', {
  ...
  spec: {
    ...
    infrastructureTemplate: {
      name: output(metalMachine.metadata).apply(x => x?.name ?? ''),
    },
  },
}, { provider });
