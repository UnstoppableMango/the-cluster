...
version="$(yq '.dependencies[] | select(.name == "cert-manager").version' "$root/Chart.yaml")"

echo "Generating CRDs for nodejs..."
crd2pulumi https://github.com/cert-manager/cert-manager/releases/download/v$version/cert-manager.crds.yaml \
