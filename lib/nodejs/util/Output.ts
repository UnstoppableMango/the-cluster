...
export const map: <A, B>(f: (a: A) => B) => (fa: Output<A>) => Output<B> =
  (f) => (fa) => fa.apply(f);

export const ap: <A>(fa: Output<A>) => <B>(fab: Output<(a: A) => B>) => Output<B> =
  (fa) => (fab) => pulumi.all([fab, fa]).apply(([f, a]) => f(a as any));
