// *** WARNING: this file was generated by crd2pulumi. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.Kubernetes.Types.Outputs.Pulumi.V1Alpha1
{

    /// <summary>
    /// PrerequisiteRef refers to another stack, and gives requirements for the prerequisite to be considered satisfied.
    /// </summary>
    [OutputType]
    public sealed class StackSpecPrerequisites
    {
        /// <summary>
        /// Name is the name of the Stack resource that is a prerequisite.
        /// </summary>
        public readonly string Name;
        /// <summary>
        /// Requirement gives specific requirements for the prerequisite; the base requirement is that the referenced stack is in a successful state.
        /// </summary>
        public readonly Pulumi.Kubernetes.Types.Outputs.Pulumi.V1Alpha1.StackSpecPrerequisitesRequirement Requirement;

        [OutputConstructor]
        private StackSpecPrerequisites(
            string name,

            Pulumi.Kubernetes.Types.Outputs.Pulumi.V1Alpha1.StackSpecPrerequisitesRequirement requirement)
        {
            Name = name;
            Requirement = requirement;
        }
    }
}
