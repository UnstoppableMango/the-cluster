// *** WARNING: this file was generated by crd2pulumi. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.Kubernetes.Types.Outputs.Controlplane.V1Alpha3
{

    [OutputType]
    public sealed class TalosControlPlaneSpecControlPlaneConfigControlplaneConfigPatches
    {
        public readonly string Op;
        public readonly string Path;
        public readonly ImmutableDictionary<string, object> Value;

        [OutputConstructor]
        private TalosControlPlaneSpecControlPlaneConfigControlplaneConfigPatches(
            string op,

            string path,

            ImmutableDictionary<string, object> value)
        {
            Op = op;
            Path = path;
            Value = value;
        }
    }
}
